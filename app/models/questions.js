...
const questionsModel = {
    ...
    /*This the function have all questions without answers*/
    ...
    async create(content) {
        ...
        /*if the query don't have find send 404*/
        if (result.rowCount === 0) {
            const resultInfo = new ResultInfos(false, 404, 'Question can\'t is created.', null);
            return resultInfo.getInfos()
        } else {
        /*else the query have find send 200*/
    ...
    async update(question, id) {
        ...
        if (result.rowCount === 0) {
            const resultInfo = new ResultInfos(false, 400, 'Can\'t update this question.', null);
            return resultInfo.getInfos();
        } else {
        /*else the query have find send 200*/
    ...
    async createAnswer(content, id, questionId) {
        ...
        if (result.rowCount === 0) {
            const resultInfo = new ResultInfos(false, 400, 'Can\'t answer created.', null);
            return resultInfo.getInfos();
        } else {
        /*else the query have find send 200*/
    ...
    /*This the function for react show thumbs on responses*/
    async haveIvoted(userId,questionId) {
        ...
        /*if have problem in database send 404*/
        if (result.rowCount === 0) {
            const resultInfo = new ResultInfos(false, 404, 'i don\'t have vote', null);
            return resultInfo.getInfos();
        } else {
        /*else the query have find send 200*/
    ...
    async unvoted(userId, questionId, answerId) {
        ...
        /*The query sql verify is user have voted for this question*/
        const queryVerify = `SELECT answer_id FROM users_has_answers
                             WHERE user_id = $1 AND
                             question_id = $2`;                 
        const resultVerify = await db.query(queryVerify, [userId, questionId]);
    ...
    /*This the function for delete answer and vote*/
    async deleteAnswerVote(questionId, answerId) { 

        /*The query sql delete answers by answers id and question id*/
        ...
        /*Else send 200*/
            const resultInfo = new ResultInfos(true, 200, 'Answer and vote delete', result.rows[0]);
            return resultInfo.getInfos();
        }
